...
def test_get_restaurants_for_city_valid_input():
    ...
    responses.add(responses.POST, "https://places.googleapis.com/v1/places:searchNearby",
                  json={"result": "success"}, status=200)

    
    response = get_restaurants_for_city(None, lat=valid_location[0], lng=valid_location[1], radius=radius,
                                        city=city_population, open_ai_client=None, included_types="restaurant")
