...
from src.data_model.place.place_subclasses import PlaceInfo, Location, Period, RegularOpeningHours, PlaceRating, \
    TimePoint
...
hotel = Place(placeInfo=PlaceInfo(displayName="Hotel"),
              location=Location(
...
def test_openingHours():
    ...
    place1 = Place(placeInfo=PlaceInfo(id='1', name='place1'),
                   ...
                   regularOpeningHours=RegularOpeningHours(periods=[
                       Period(open=TimePoint(hour=10), close=TimePoint(hour=18)) for _ in range(7)
                   ]))

    place2 = Place(placeInfo=PlaceInfo(id='2', name='place2'),
                   location=AGH_location,
                   ratings=PlaceRating(rating=4.5, confidenceRating=4.5),
                   regularOpeningHours=RegularOpeningHours(periods=[
                       Period(open=TimePoint(hour=11), close=TimePoint(hour=18)) for _ in range(7)
                   ]))

    place3 = Place(placeInfo=PlaceInfo(id='3', name='place3'),
                   location=Rynek_location,
                   ratings=PlaceRating(rating=4.5, confidenceRating=4.5),
                   regularOpeningHours=RegularOpeningHours(periods=[
                       Period(open=TimePoint(hour=11), close=TimePoint(hour=17)) for _ in range(7)
                   ]))
...
def test_closing_hours():
    ...
    place3 = Place(placeInfo=PlaceInfo(id='3', name='place3'),
                   ...
                   ratings=PlaceRating(rating=4.5, confidenceRating=4.5),
                   regularOpeningHours=RegularOpeningHours(periods=[
                       Period(open=TimePoint(hour=10), close=TimePoint(hour=17)) for _ in range(7)
                   ]))
...
def test_order():
    place1 = Place(placeInfo=PlaceInfo(id='1', displayName='place1'),
                   location=Wawel_location,
                   ratings=PlaceRating(rating=4.5, confidenceRating=4.5),
                   regularOpeningHours=RegularOpeningHours(periods=[
                       Period(open=TimePoint(hour=11), close=TimePoint(hour=14)) for _ in range(7)
                   ]))

    place2 = Place(placeInfo=PlaceInfo(id='2', displayName='place2'),
                   location=AGH_location,
                   ratings=PlaceRating(rating=4.5, confidenceRating=4.5),
                   regularOpeningHours=RegularOpeningHours(periods=[
                       Period(open=TimePoint(hour=12), close=TimePoint(hour=15)) for _ in range(7)
                   ]))

    place3 = Place(placeInfo=PlaceInfo(id='3', displayName='place3'),
                   location=Rynek_location,
                   ratings=PlaceRating(rating=4.5, confidenceRating=4.5),
                   regularOpeningHours=RegularOpeningHours(periods=[
                       Period(open=TimePoint(hour=13), close=TimePoint(hour=16)) for _ in range(7)
                   ]))
...
def test_dropping_places_with_lower_rating():
    ...
    place1 = Place(placeInfo=PlaceInfo(id='1', displayName='place1'),
                   ...
                   regularOpeningHours=RegularOpeningHours(periods=[
                       Period(open=TimePoint(hour=11), close=TimePoint(hour=12)) for _ in range(7)
                   ]))
...
def test_open_is_close():
    ...
    place1 = Place(placeInfo=PlaceInfo(id='1', displayName='place1'),
                   ...
                   ratings=PlaceRating(rating=4.0, confidenceRating=4.0),
                   regularOpeningHours=RegularOpeningHours(periods=[
                       Period(open=TimePoint(hour=11), close=TimePoint(hour=11)) for _ in range(7)
                   ]))
    ...
    place2 = Place(placeInfo=PlaceInfo(id='2', displayName='place2'),
                   ...
                   ratings=PlaceRating(rating=4.5, confidenceRating=4.5),
                   regularOpeningHours=RegularOpeningHours(periods=[
                       Period(open=TimePoint(hour=12), close=TimePoint(hour=12)) for _ in range(7)
                   ]))
    places = Places([place1, place2], city)
...
def test_real_data():
    ...
    places_good_hours = Places([place for place in places.get_list()
                                if place.regularOpeningHours.periods[0].open_in_minutes <
                                place.regularOpeningHours.periods[0].close_in_minutes], places.city)
    ...
    for i, places in enumerate(clustered_places):
        ...
        route = Routing(places, depot=hotel, num_routes=1, period_index=period).get_routes()[0]
