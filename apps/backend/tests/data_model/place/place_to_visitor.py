...
from src.data_model.place.place_subclasses import PlaceInfo, TextWithLang, Location, PlaceRating
...
class TestPlaceToItinerary(unittest.TestCase):
    ...
    def test_place_to_itinerary(self):
        ...
        self.assertEqual(itinerary['formattedAddress'], self.place.placeInfo.formattedAddress)
        ...
        self.assertEqual(itinerary['image'], self.place.photos[0] if len(self.place.photos) > 0 else None)
    ...
    def test_check_types_good_types(self):
        good_types = ['restaurant', 'museum', 'amusement_park', 'aquarium', 'art_gallery', 'bowling_alley', 'casino',
                      'movie_theater', 'night_club', 'zoo']
    ...
    def test_check_types_excluded_categories(self):
        excluded_categories = ['airport', 'bus_station', 'car_rental', 'car_repair', 'car_wash', 'gas_station',
                               'parking', 'subway_station', 'taxi_stand', 'train_station', 'transit_station']  
