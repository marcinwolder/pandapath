...
from src.data_model.place.place_subclasses import PlaceInfo, PlaceRating
	...
	return Places(
		[
			Place(
				...
				ratings=PlaceRating(
					rating=4.0, cumulative_rating=4.0, userRatingCount=50.0
				),
			),
			Place(
				placeInfo=PlaceInfo(id='2'),
				ratings=PlaceRating(
					rating=4.0, cumulative_rating=5.0, userRatingCount=4.0
				),
			),
		]
	)
...
class TestStatisticalRating(unittest.TestCase):
	...
	def test_calculate_statistical_rating(self):
		...
		for place in self.places.get_list():
			print(place.ratings)
		assert (
			self.places.get_place_by_id('1').ratings.statisticalRating
