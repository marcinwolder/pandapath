...
def _make_restaurants():
	...
	for i in range(5):
		restaurant = Place(
			placeInfo=PlaceInfo(id=f'restaurant{i}', displayName=f'restaurant{i}'),
			estimatedTime=60,
		)
...
def test_get_ratings():
	with patch('src.rating.restaurants.Rating.get_ratings') as mock_get_ratings:
...
def test_get_prices():
	with patch('src.rating.restaurants.Price.get_prices') as mock_get_prices:
