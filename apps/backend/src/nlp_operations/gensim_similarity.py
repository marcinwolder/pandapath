...
from src.constants import (default_subcategories,
                           dining, all_categories, dining_subcategories)

# TODO: refactor

def calculate_similarity(words: List[Tuple[str, int]], model: gensim.models.keyedvectors.Word2VecKeyedVectors):
    """Calculate similarity between words and categories using word2vec.
    ...
    for word_with_sentiment in words:
        ...
        for category in lst:
            if (model.similarity(word_with_sentiment,
                                 category) >  # a to nie powinno być model.wv.similarity('france', 'spain')?
                    map_words_to_categories[word_with_sentiment][0]):
...
def get_similarity3(words: List[Tuple[str, int]], model: object, categories: List[str] = None,
