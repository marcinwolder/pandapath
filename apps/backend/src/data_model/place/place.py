...
class Place(ToJson):
    ...
    ratings: PlaceRating = PlaceRating()
    reviews: List[Review] = field(default_factory=list)
    editorialSummary: TextWithLang = TextWithLang()
    ...
    priceLevel: int = 0
    accessibilityOptions: AccessibilityOptions = AccessibilityOptions()
...
class PlaceCreator(ABC):
    ...
    def _create_place(self):
        ...
        place.location = Location(
            **self.data['location']) if 'location' in self.data else Location()  

        if 'regularOpeningHours' in self.data:  
            place.regularOpeningHours = RegularOpeningHours(periods=self.data['regularOpeningHours']['periods'],
        ...
        place.primaryType = self.data.get('primaryType', 'tourist_attraction'
        if 'tourist_attraction' in place.types else '')
        ...
        place.accessibilityOptions = AccessibilityOptions(**self.data.get('accessibilityOptions', {}))
        place.reviews = [Review(**{k: v for k, v in review.items()
                                   if k in Review.__annotations__}) for review in
                         self.data.get('reviews', [])]
        place.photos = [Photo(**{k: v for k, v in photo.items()
                                 if k in Photo.__annotations__}) for photo in
                        self.data.get('photos', [])]
...
class PlaceCreatorAPI(PlaceCreator):
    ...
    def create_place(self):
        ...
        place.regularOpeningHours.test = self.data.get('regularOpeningHours', {})  

        place.placeInfo = PlaceInfo(name=self.data.get('name', ''), id=self.data.get('id', ''),
        ...
        place.dining = Dining(reservable=self.data.get('reservable', False), takeout=self.data.get('takeout', False),
                              dineIn=self.data.get('dineIn', False), delivery=self.data.get('delivery', False),
        ...
        place.alcohol = Alcohol(servesBeer=self.data.get('servesBeer', False),
                                servesWine=self.data.get('servesWine', False),
                                servesCocktails=self.data.get('servesCocktails', False))

        place.children = Children(goodForChildren=self.data.get('goodForChildren', False),
        ...
        place.ratings = PlaceRating(statisticalRating=self.data.get('statisticalRating', 0),
...
class PlaceCreatorDatabase(PlaceCreator):
    ...
    def create_place(self):
        ...
        place.reviews = [Review(**{k: v for k, v in review.items() if k in Review.__annotations__}) for review in
                         self.data.get('reviews', [])]
        return place
