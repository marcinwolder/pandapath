...
from src.constants import excluded_categories, dining, all_categories, default_categories
...
class Visitor:
    ...
    def place_to_itinerary(place: Place, transportation: [int, str] = None):
        ...
        return {"name": place.placeInfo.displayName,
    ...
    def is_suitable(self, place, population):
        ...
        if (place.placeInfo.businessStatus == 'CLOSED_PERMANENTLY'
        ...
        if place.ratings.confidenceRating < 3.5:
            print("Place has too low confidence rating: ", place.ratings.confidenceRating)
            return False
    ...
    @staticmethod
    def _save_places_to_database(db, places: Places, city: City, category_type: str, place_type: str,
                                 categories: List[str]):
        db.add_categories_to_database(city=city, category_type=category_type, categories=categories)
        for place in places.get_list():
            db.add_place_to_database(place, city, category_type, place_type, categories)

    @abstractmethod
    def save_places_to_database(self, db, places: Places, city: City,
                                categories: List[str]):
        pass
...
class PlaceVisitor(Visitor):
    ...
    def save_places_to_database(self, db, places: Places, city: City,
                                categories: List[str]):
        self._save_places_to_database(db, places, city, "places_categories",
                                      "places", categories)
...
class RestaurantVisitor(Visitor): 
    ...
    def _check_types(self, place):
        ...
        :return: True if the place is suitable, False otherwise
        """
        return super()._check_types(place) and any(d_type in place.types for d_type in dining)

    def save_places_to_database(self, db, places: Places, city: City,
                                categories: List[str]):
        self._save_places_to_database(db, places, city, "restaurant_categories",
