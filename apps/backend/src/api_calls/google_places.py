...
from src.constants import default_categories, excluded_categories, all_categories
from src.data_model import City, Places
from src.data_model.place.place import PlaceCreatorAPI, PlaceCreator
...
def nearby_search(location, included_primary_types=None, included_types=None, excluded_types=None, radius=20000) \
        -> Response:
...
def get_restaurants_for_city(db, lat, lng, radius, city, vegan=False, included_types=None) -> Places:
    if included_types is None:
        included_types = ["restaurant"]
    if vegan:
        included_types.append("vegan_restaurant")
    places_list = []
    search_results = nearby_search(location=(lat, lng),
    ...
    try:
        ...
        for result in search_results['places']:
            ...
            if not RestaurantVisitor().is_suitable(place=place, population=city.population):
                continue
        ...
        logging.error("KeyError: %s", search_results)
...
def get_places_for_city(db, city: City,
    ...
    def process_search_results(search_results):
        try:
            ...
            for result in search_results['places']:
                ...
                if not placeVisitor.is_suitable(place=place, population=city.population):
                    continue
                places_list.append(place)
        except KeyError:
            logging.error("KeyError: %s", search_results)

    process_search_results(nearby_search(location=(city.lat, city.lng),
                                         included_types=['tourist_attraction'],
                                         excluded_types=excluded_categories,
                                         radius=city.get_radius()).json())
    for category in search_categories:
        search_results = (nearby_search(location=(city.lat, city.lng),
