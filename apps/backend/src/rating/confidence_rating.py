...
def confidence_rating_by_population(rating, user_rating_count, population=200000):
	...
	confidence_interval = (
		...
		* (
			(
				avg_rating_normalized * (1 - avg_rating_normalized)
				+ z**2 / (4 * user_rating_count)
			)
			/ user_rating_count
		)
		** 0.5
	)
	lower_bound = (avg_rating_normalized + factor - confidence_interval) / (
		1 + z**2 / user_rating_count
	)
