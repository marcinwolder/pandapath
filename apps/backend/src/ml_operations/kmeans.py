...
def kmens_plot(data, cluster_labels, cluster_centers, labels):
    ...
    plt.scatter(data[:, 1], data[:, 0], c=cluster_labels,
                cmap='viridis')  
    plt.scatter(cluster_centers[:, 1], cluster_centers[:, 0], c='red',
                marker='X')  
...
def kmeans_clustering(places: Places, days: int = 3) -> List[Places]:
    ...
    coordinates = np.array([[place.location.latitude, place.location.longitude] for place in places.get_list()])
    ...
    trips = [places.create_new_places_with_id_list(places_ids) for places_ids in places_ids_list]
