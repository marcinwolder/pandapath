...
def _get_places_and_save_to_file(city):
    ...
    db = None
    attractions = get_places_for_city(db, city, PlaceCreatorAPI,
                                      PlaceVisitor, save_raw=False)
    with open(get_path('attractions.json', 'data'), 'w') as f:
        json.dump(attractions.get_list(), f, default=vars)
    return attractions


def get_attractions(db: DataBase | None, city: City, user_preferences, from_file: bool = False) -> Places:
    ...
    if db.check_if_city_exist(city):
        ...
        attractions = get_places_for_city(db, city, placeCreator=PlaceCreatorAPI, placeVisitor=PlaceVisitor)
...
def get_recommendations(db: DataBase, db_users: DataBaseUsers, user_id: str, city_id: int, days: int,
                        dates: [date, date], preferences: UserPreferences, from_file: bool = False):
    """Function that returns a list of place for each day."""

    city = City(city_id)
    
    user = TripInfo(user_id=user_id, user_preferences=preferences, city=city, days=days, dates=dates)
