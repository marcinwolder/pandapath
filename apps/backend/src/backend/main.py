...
from src.backend.get_recommendation import get_attractions, get_recommendations
...
parser.add_argument(
    "-f", "--from_file", action="store_true", help="Read input from file"
)
parser.add_argument(
    "-n", "--no_db", action="store_true", help="Disable database interaction"
)
...
async def social_media():
    ...
    user_specified_needs = UserPreferences(
        data["user_preferences"], user_preferences, subcategories=[]
    )
    recommendation, city_overview = get_recommendations(
        db, data, user_specified_needs, from_file=from_file
    )
...
def run_debug(data):
    ...
    user_specified_needs = UserPreferences(
        data["user_preferences"], categories, subcategories
    )
    recommendation = get_recommendations(
        db, data, user_specified_needs, from_file=from_file
    )
