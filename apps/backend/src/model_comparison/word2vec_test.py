...
from src.constants import (all_categories, default_categories,
                           dining, default_subcategories, dining_subcategories)
import time

from src.model_comparison.utils import save





model_names = ["glove-twitter-25", "glove-twitter-50", "glove-twitter-100", "glove-twitter-200",
               "glove-wiki-gigaword-50", "glove-wiki-gigaword-100", "glove-wiki-gigaword-200",
               "glove-wiki-gigaword-300", "GoogleNews-vectors-negative300",
...
def single_label_classification(model):
    ...
    for category in categories:
        ...
        accuracy_dict[category] = (sum([1 for word in test_dict if test_dict[word][1] == category])
                                   / len(test_dict))
...
def load_models(model_name):
    ...
    avg_accuracy, accuracy_dict, missing_words, missing_word_ratio = single_label_classification(
        model)
