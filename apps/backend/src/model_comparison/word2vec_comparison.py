...
def plot(model_dict):
    ...
    names_dict = {"glove-twitter-25": "tw-25",
    ...
    _plot('Czas ładowania modeli', model_names, loading_times, 'skyblue', 'Czas (s)', 'loading_times')
    _plot('Czas wnioskowania modeli', model_names, inference_times, 'lightgreen', 'Czas (s)', 'inference_times')
    _plot('Dokładność modeli', model_names, accuracies, 'salmon', 'Dokładność', 'accuracies')
    _plot('Brakujące słowa', model_names, missing_words, 'lightcoral', 'Brakujące słowa', 'missing_words')


def main():
    model_dict = "word2vec/results.txt"
    with open(model_dict, "r") as file:
