...
class TravelEstimator:
    def __init__(self, car_model_path=get_path('car_model.joblib', 'time_estimation_models'),
    ...
    def get_estimated_time(self, start_coordinates: Location, end_coordinates: Location,
                           city: City = None):
        """Zwraca czas w minutach i środek transportu"""
        distance = calculate_distance(start_coordinates.latitude, start_coordinates.longitude,
                                      end_coordinates.latitude, end_coordinates.longitude)
        if distance > 5:
            
            if city is not None and city.country == 'Poland':
                try:
                    return int(self.get_travel_time(start_coordinates, end_coordinates)/60) * 2, 'CAR'
    ...
    @staticmethod
    def get_travel_time(start_coordinates, end_coordinates, server_url="http://127.0.0.1:5002"):
        ...
        coordinates = (f"{start_coordinates.longitude},{start_coordinates.latitude};"
        ...
        if response.status_code == 200:
            ...
            raise Exception(f"Error: Unable to fetch data from OSRM. Status code: {response.status_code}")
...
if __name__ == '__main__':
    ...
    print(travel_estimator.get_estimated_time(
