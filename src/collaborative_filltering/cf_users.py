...
from src.data_model.users.users import Users
...
def find_top_n_neighbors(user_similarity_df, n_neighbors):
    """
        ...
        """
    top_neighbors = np.argsort(-user_similarity_df.values, axis=1)[:, 1:n_neighbors + 1]
    top_neighbors_df = pd.DataFrame(top_neighbors, index=user_similarity_df.index,
                                    columns=[f"Neighbor_{i + 1}" for i in range(n_neighbors)])

    return top_neighbors_df


def predict_ratings_with_neighbors(ratings_matrix, user_similarity_df, top_neighbors_df):
    """
...
def main():
    ...
    ratings_pred = predict_ratings_with_neighbors(ratings_matrix, user_similarity_df, top_neighbors_df)
