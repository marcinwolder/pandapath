...
class StatisticalRating:
    ...
    def calculate_bayesian_rating(self, place, average_review_count, average_rating):
        ...
        """
        print(place.placeInfo.id, average_review_count, average_rating, place.ratings.userRatingCount, place.ratings.rating)
        sum_review_count = average_review_count + place.ratings.userRatingCount
        print(sum_review_count)
        if sum_review_count == 0:
            return 0
        return (
                average_review_count * average_rating + place.ratings.userRatingCount * place.ratings.rating) / sum_review_count
    ...
    def average_rating(self) -> float:
        ...
        if self._average_rating == -1:
            self._average_rating = self._calculate_average(lambda place: place.ratings.cumulative_rating)
    ...
    def average_rating_count(self) -> float:
        ...
        if self._average_rating_count == -1:
            self._average_rating_count = self._calculate_average(lambda place: place.ratings.userRatingCount)
        return self._average_rating_count

    def calculate_statistical_rating(self):
        """Returns a Bayesian normalized rating of all places"""
        for place in self._places.values():
            place.ratings.statisticalRating = (
                self.calculate_bayesian_rating(place, self.average_rating_count, self.average_rating))
